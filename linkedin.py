...
class Linkedin(object):
    ...
    def get_job_description(self, job_id: str) -> Dict:
        ...
        try:
            ...
            data = res.json()
            if data and "status" in data and data["status"] != 200:
                self.logger.info(f"request failed: {data.get('message', 'Unknown error')}")
                return {}
            ...
            job_posting["title"] = data.get("title", "")
            job_posting["description"] = data.get("description", {}).get("text", "")
            ...
            if "companyDetails" in data:
                ...
                job_posting["company"] = {
                    ...
                    "description": company.get("company", {}).get("description", ""),
                }

            # Get listed date and expiry
            job_posting["listed_date"] = data.get("listedAt", "")
            job_posting["expiry_date"] = data.get("expireAt", "")
        
            # Get salary information if available
            if "salaryInsights" in data:
                salary = data["salaryInsights"]
                job_posting["salary"] = {
                    "min": salary.get("compensationRange", {}).get("min", ""),
                    "max": salary.get("compensationRange", {}).get("max", ""),
                    "currency": salary.get("compensationRange", {}).get("currency", ""),
                    "period": salary.get("compensationRange", {}).get("period", "")
                }
            
            # Get skills requirements if available
            if "requiredSkills" in data:
                job_posting["required_skills"] = data["requiredSkills"]
            
            # Get application type and URL
            job_posting["apply_url"] = data.get("applyUrl", "")
            job_posting["application_type"] = data.get("applicationType", "")
        
            return job_posting
        
        except Exception as e:
            self.logger.error(f"Error fetching job description for job ID {job_id}: {str(e)}")
            return {}
